...
export class QuestionairesService {
    ...
    return this.questionairesDataProvider.getAllQuestionaires(matchString).pipe(
      tap((questionaires) => {
        if (questionaires.length > 0) {
          if (matchString) {
            this.log(`Found questionaires matching "${matchString}"`);
          } else {
            this.log('Fetched all questionaires');
          }
        } else if (matchString) {
          this.log(`Did not find any questionaires matching "${matchString}"`);
        } else {
          this.log('There are no questionaires to fetch');
        }
      }),
      catchError((err: IErrReport) => {
        ...
        this.log('ERROR: Failed to get questionaires from server');
      ...
      .pipe(
        tap((questionaires) => {
          if (questionaires.length > 0) {
            ...
            this.log(
              `Did not find any questionaires matching "${matchString}"`,
            );
          } else {
            this.log('There are no questionaires to fetch');
          }
  ...
  getQuestionaire(QuestionaireId: number): Observable<IQuestionaire> {
    ...
    return this.questionairesDataProvider.getQuestionaire(QuestionaireId).pipe(
      tap((_) => {
        this.log(`Fetched questionaire with id = ${QuestionaireId}`);
      }),
      ...
      catchError((errReport: IErrReport) => {
        ...
        if (
          ...
          this.log(
            `ERROR: Did not find questionaire with id = ${QuestionaireId}`,
          );
        } else {
          /* otherwise a general fail */
          this.log('ERROR: Failed to get questionaire from server');
        }
        this.toastr.error('ERROR!', this.toastrMessage);
        /* mark as handled */
  ...
  addQuestionaire(
    ...
    return this.questionairesDataProvider.addQuestionaire(questionaire).pipe(
      tap((newQuestionaire: IQuestionaire) => {
        this.log(`Added questionaire with id = ${newQuestionaire.id}`);
      }),
      ...
      catchError((err: IErrReport) => {
        ...
        /* inform user and mark as handled */
        this.log('ERROR: Failed to add questionaire to server');
        this.toastr.error('ERROR!', this.toastrMessage);
        err.isHandled = true;
  ...
  deleteQuestionaire(
    ...
    return this.questionairesDataProvider
      ...
      .pipe(
        tap(() => {
          this.log(`Deleted questionaire with id = ${questionaireId}`);
        }),
        ...
        catchError((errReport: IErrReport) => {
          ...
          if (
            ...
            /* otherwise a general fail */
            this.log('ERROR: Failed to delete questionaire from server');
            this.toastr.error('ERROR!', this.toastrMessage);
          }
          /* mark as handled */
          errReport.isHandled = true;
  ...
  updateQuestionaire(questionaire: IQuestionaire): Observable<IQuestionaire> {
    ...
    return this.questionairesDataProvider.updateQuestionaire(questionaire).pipe(
      tap(() => {
        this.log(`Updated questionaire with id = ${questionaire.id}`);
      }),
      ...
      catchError((errReport: IErrReport) => {
        ...
        if (
          ...
          /* 404: questionaire did not exist */
          this.log(
            `ERROR: Did not find questionaire with id = ${questionaire.id}`,
          );
        } else {
          /* otherwise a general fail */
          this.log('ERROR: Failed to update questionaire on the server');
          this.toastr.error('ERROR!', this.toastrMessage);
        }
        /* mark as handled */
        errReport.isHandled = true;
  ...
  private log(message: string): void {
    this.logger.trace(`${QuestionairesService.name}: Reporting: ${message}`);
    this.messageService.add(`QuestionairesService: ${message}`);
  }
}
