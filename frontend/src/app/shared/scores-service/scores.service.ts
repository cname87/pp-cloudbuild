...
import { IScores } from '../../data-providers/models/models';
...
export class ScoresService {
    ...
    return this.scoresDataProvider.getOrCreateScores(memberId, date).pipe(
      ...
      catchError((errReport: IErrReport) => {
        ...
        if (
          ...
          this.log('ERROR: Failed to get scores table from server');
        }
  ...
  updateScoresTable(scores: IScores): Observable<IScores> {
    ...
    return this.scoresDataProvider.updateScoresTable(scores).pipe(
      tap((data: IScores) => {
        this.log(`Updated scores table with date = ${data.date.toISOString()}`);
      }),
      ...
      catchError((errReport: IErrReport) => {
        ...
        if (
          ...
          /* 404: scores table did not exist */
          this.log(`ERROR: Did not find scores table with id = ${scores.id}`);
        } else {
          /* otherwise a general fail */
          this.log('ERROR: Failed to update scores table on the server');
          this.toastr.error('ERROR!', this.toastrMessage);
        }
        /* mark as handled */
        errReport.isHandled = true;
  ...
  private log(message: string): void {
    this.logger.trace(`${ScoresService.name}: Reporting: ${message}`);
    this.messageService.add(`ScoresService: ${message}`);
  }
}
