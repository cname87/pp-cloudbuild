...
import { IScores } from '../../data-providers/models/scores';
import { RouteStateService } from '../../shared/route-state-service/router-state-service';
import { ScoresService } from '../../shared/scores-service/scores.service';
...
export class MemberScoresComponent implements OnDestroy {
  ...
  /* The member retrieved from the route data */
  member$ = of({}) as Observable<IMember>;
  /* ngx-formly form definition variables */
  form = new FormGroup({});
      ...
      fieldGroup: [
        {
          key: 'date',
          type: 'datepicker',
          /* A date is entered as midnight local time but is stored as a UTC string.  UTC might be different from local time, e.g. one hour behind which means the UTC value is 23.00 of the day before which means the date stored could be a time during the day before. This causes problems if you pass the UTC value to calculate the date (e.g. July 4th). To avoid this, subtract the local offset from UTC from the stored value. EG: If IST is 60min ahead of UTC then July 4th 00:00 IST is stored as July 3rd 23:00 UTC.  In this case the IST offset from UTC is -60 min.  Subtracting -60min from the stored value results in the updated stored UTC value being July 4th 00:00 UTC, or July 4th 01:00 IST - both are on the 'correct' date of July 4th. */
          parsers: [
            (date) => {
              return new Date(
  ...
  constructor(
    ...
    /* Note: loading in constructor to avoid angular change after checked */
  ...
  /**
   * After every table data change, (i.e. excluding the date), the data is sent to the database and an event is emitted to the datatable type, (which redraws the table).
